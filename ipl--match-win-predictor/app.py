...
def predict():
    ...
    win_percentage = prediction[0] * 100
    loss_percentage = (1 - prediction[0]) * 100

    # Render the HTML template with the prediction result
    return render_template('result.html', batting_team=batting_team, bowling_team=bowling_team, win=win_percentage, loss=loss_percentage)
